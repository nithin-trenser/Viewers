...
function PanelMeasurementTableTracking({ servicesManager, extensionManager }) {
  ...
  const onMeasurementItemEditHandler = ({ uid, isActive }) => {
    ...
    if (measurement?.autoGenerated) {
      uiNotificationService.show({
        title: 'Non-Relabelable Measurement',
        message: 'Interpolated measurements cannot be relabelled',
        type: 'info',
        duration: 3000,
      });
    ...
    showLabelAnnotationPopup(uid, uiDialogService, modeLabelConfig, measurementService).then(
      (val: Map<any, any>) => {
