import { triggerEvent, eventTarget, Enums as csCoreEnums } from '@cornerstonejs/core';
...
const initMeasurementService = (
  ...
  measurementService.addMapping(
    csTools3DVer1MeasurementSource,
    'OpenFreehandROI',
    OpenFreehandROI.matchingCriteria,
    OpenFreehandROI.toAnnotation,
    OpenFreehandROI.toMeasurement
  );

  measurementService.addMapping(
    csTools3DVer1MeasurementSource,
    'ClosedFreehandROI',
    ClosedFreehandROI.matchingCriteria,
    ClosedFreehandROI.toAnnotation,
    ClosedFreehandROI.toMeasurement
  );

  measurementService.addMapping(
    csTools3DVer1MeasurementSource,
    'ContourROI',
    ContourROI.matchingCriteria,
    ContourROI.toAnnotation,
...
const connectToolsToMeasurementService = servicesManager => {
  ...
  function elementEnabledHandler(event) {
    ...
    element.addEventListener(
      interpolationCompletedEvt,
      addOrUpdateInterpolatedMeasurements.bind(null, element)
    );
    element.addEventListener(interpolationDeletedEvt, deleteInterpolatedMeasurements);
  }

  function elementDisabledHandler(event) {
    const { element } = event.detail;

    element.removeEventListener(interpolationCompletedEvt, addOrUpdateInterpolatedMeasurements);
    element.removeEventListener(interpolationDeletedEvt, deleteInterpolatedMeasurements);
  }

  /**
   * When csTools fires a interpolation completed event, add or update the measurement
   * from the measurement service
   *
   * @param {*} csToolsEvent
   */
  function addOrUpdateInterpolatedMeasurements(element, csToolsEvent) {
    const { annotation } = csToolsEvent.detail;
    const { interpolationUID } = annotation;

    const annotations = getAnnotations(toolNames.ContourROITool, element);
    const measurements = measurementService.getMeasurements();

    annotations.forEach(annotation => {
      if (annotation.interpolationUID === interpolationUID && annotation.autoGenerated) {
        const matchingMeasurement = measurements.find(
          measurement => measurement.annotationUID === annotation.annotationUID
        );
        ...
        matchingMeasurement ? updateMeasurement(event) : addMeasurement(event);
      }
    });
  }

  /**
   * When csTools fires a interpolation removed event, remove the same measurement
   * from the measurement service
   *
   * @param {*} csToolsEvent
   */
  function deleteInterpolatedMeasurements(csToolsEvent) {
    const { annotations } = csToolsEvent.detail;
  ...
  const elementEnabledEvt = csCoreEnums.Events.ELEMENT_ENABLED;
  const elementDisabledEvt = csCoreEnums.Events.ELEMENT_DISABLED;
  const interpolationCompletedEvt = csToolsEvents.ANNOTATION_INTERPOLATION_PROCESS_COMPLETED;
  const interpolationDeletedEvt = csToolsEvents.INTERPOLATED_ANNOTATIONS_REMOVED;
  ...
  eventTarget.addEventListener(elementEnabledEvt, elementEnabledHandler);
  eventTarget.addEventListener(elementDisabledEvt, elementDisabledHandler);
  ...
  measurementService.subscribe(
    ...
    ({ source, measurement, notYetUpdatedAtSource }) => {
      ...
      if (data.label !== label) {
        ...
        const enbledElement = getActiveViewportEnabledElement(viewportGridService);
        const { viewportId, renderingEngineId } = enbledElement;
        const eventDetail = {
        ...
        triggerEvent(eventTarget, csToolsEvents.ANNOTATION_LABEL_CHANGE, eventDetail);
      }
