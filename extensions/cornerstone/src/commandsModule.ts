...
function commandsModule({
  ...
  const actions = {
    ...
    showCornerstoneContextMenu: options => {
      const isInterpolatedMeasurement =
        options.nearbyToolData?.interpolationUID && options.nearbyToolData?.autoGenerated;
      // Prevent context menu for auto generated annotations
      if (isInterpolatedMeasurement) {
        return;
      }
    ...
    setMeasurementLabel: ({ uid }) => {
      ...
      showLabelAnnotationPopup(uid, uiDialogService, labelConfig, measurementService).then(
        (val: Map<any, any>) => {
